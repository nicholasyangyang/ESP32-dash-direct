// Copied from ref/szpi-s3-esp/08-lcd_lvgl/main/esp32_s3_szp.c — Tesla dashboard BSP
#include <stdio.h>
#include "bsp.h"
...
{
    /* 初始化触摸屏 — 失败时警告继续，不 abort */
    esp_err_t ret = bsp_touch_new(&tp);
    if (ret != ESP_OK || tp == NULL) {
        ESP_LOGW(TAG, "Touch init failed (err=0x%x), running without touch", ret);
        return NULL;
    }
